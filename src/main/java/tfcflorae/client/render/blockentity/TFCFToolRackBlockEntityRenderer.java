...
import tfcflorae.common.blocks.wood.TFCFToolRackBlock;

public class TFCFToolRackBlockEntityRenderer implements BlockEntityRenderer<TFCFToolRackBlockEntity>
{
    ...
    @Override
    public void render(TFCFToolRackBlockEntity toolRack, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay)
    {
        if (toolRack.getLevel() == null) return;
        toolRack.getCapability(Capabilities.ITEM).ifPresent(cap -> {
            int meta = toolRack.getBlockState().getValue(TFCFToolRackBlock.FACING).get2DDataValue();
