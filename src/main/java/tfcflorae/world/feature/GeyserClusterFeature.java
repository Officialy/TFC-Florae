...
public class GeyserClusterFeature extends Feature<GeyserClusterConfig>
{
   public GeyserClusterFeature(Codec<GeyserClusterConfig> codec)
   {
      super(codec);
   }

   @Override
   public boolean place(FeaturePlaceContext<GeyserClusterConfig> context)
   {
      ...
      else
      {
         ...
         for(int l = -j; l <= j; ++l)
         {
            for(int i1 = -k; i1 <= k; ++i1)
            {
               ...
               this.placeColumn(state.get(), hasSurface, surfaceState.get(), level, random, pos1, l, i1, f, d0, i, f1, config);
            }
         }
         return true;
      }
   ...
   private double getChanceOfStalagmiteOrStalactite(int xRadius, int zRadius, int x, int z, GeyserClusterConfig config)
   {
